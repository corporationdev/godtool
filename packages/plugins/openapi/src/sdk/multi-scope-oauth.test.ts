...
layer(TestLayer)("OpenAPI multi-scope OAuth", (it) => {
  it.effect("per-user Connections coexist with a shared org-level client credential", () =>
    Effect.gen(function* () {
      ...
      const aliceStart = yield* aliceExec.openapi.startOAuth(startInputFor("alice", aliceScope.id));
      const bobStart = yield* bobExec.openapi.startOAuth(startInputFor("bob", bobScope.id));
