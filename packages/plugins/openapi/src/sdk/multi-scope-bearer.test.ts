...
layer(TestLayer)("OpenAPI multi-scope bearer (Vercel-style)", (it) => {
  it.effect("admin-added source; each user's per-scope token wins on invocation", () =>
    Effect.gen(function* () {
      ...
      const aliceToken = aliceRows.find((r) => (r.id as unknown as string) === "vercel_api_token");
      expect(aliceToken?.scopeId as unknown as string).toBe("user-alice");

      const bobRows = yield* bobExec.secrets.list();
      const bobToken = bobRows.find((r) => (r.id as unknown as string) === "vercel_api_token");
  ...
  it.effect("user-scope source shadows inherit the org baseUrl", () =>
    Effect.gen(function* () {
      ...
      const source = yield* aliceExec.openapi.getSource("vercel", aliceScope.id as string);
