...
export const MANAGED_AUTH_PROVIDER_KEY = "managed-auth:composio" as const;
...
export type ManagedAuthConfig = ComposioManagedAuthConfig;
...
export const invokeManagedHttp = (input: {
  ...
  Effect.gen(function* () {
    ...
    if (!connection) {
      return yield* new ManagedAuthInvocationError({ message: "Managed auth connection is missing" });
    }
    ...
    if (typeof connectedAccountId !== "string" || connectedAccountId.length === 0) {
      return yield* new ManagedAuthInvocationError({ message: "Managed auth connection is missing its account id" });
    }

    if (typeof providerState.brokerUrl === "string" && providerState.brokerUrl.length > 0) {
      const brokerToken = yield* input.connections
        .accessToken(input.config.connectionId)
        .pipe(
          Effect.mapError(
            () => new ManagedAuthInvocationError({ message: "Managed auth broker token lookup failed" }),
          ),
        );
      return yield* Effect.tryPromise({
        try: async () => {
          ...
          const body = (await response.json().catch(() => null)) as ManagedHttpResult | null;
          if (!response.ok) {
            throw new ManagedAuthInvocationError({
              message:
                body && typeof body.error === "string"
                  ? body.error
                  : `Managed auth broker failed with status ${response.status}`,
            });
          }
          if (!body) {
            throw new ManagedAuthInvocationError({
              message: "Managed auth broker returned no body",
            });
          }
          return body;
        },
