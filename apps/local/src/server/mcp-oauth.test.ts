...
import { addGroup, InternalError, observabilityMiddleware } from "@executor/api";
...
type TestApiShape =
  typeof TestApi extends HttpApi.HttpApi<infer _Id, infer Groups, infer ApiError, infer _ApiR>
...
const startHarness = async (tmpDir: string): Promise<Harness> => {
  ...
  const { handler: webHandler, dispose: disposeHandler } = HttpApiBuilder.toWebHandler(
    TestApiBase.pipe(
